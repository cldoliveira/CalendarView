...
class CompactCalendarController {
    ...
    //dados modificados
    private int calenderHeaderTextColor=Color.BLACK;
    private float smallIndicatorRadiusFactor=1;

    //Novos atributos para a mudança de Typeface
    private Typeface typeface_header=Typeface.DEFAULT_BOLD;
    private Typeface typeface_body=Typeface.DEFAULT;
    ...
    //Raio do indicador com valor entre 2.5 e 7
    public void setIndicatorRadius(float indicatorRadius) {
        if(indicatorRadius>=2.5 && indicatorRadius<=7){
            smallIndicatorRadius=indicatorRadius * screenDensity;
            this.smallIndicatorRadiusFactor=indicatorRadius/2.5f;
        } else if(indicatorRadius < 2.5) {
            smallIndicatorRadius=2.5f * screenDensity;
            this.smallIndicatorRadiusFactor=1;
        } else if(indicatorRadius > 7) {
            smallIndicatorRadius=7*screenDensity;
            this.smallIndicatorRadiusFactor=7.0f/2.5f;
        }
    }
    ...
    void setCalendarHeaderTextColor(int calenderHeaderTextColor) {
        this.calenderHeaderTextColor=calenderHeaderTextColor;
    }
    ...
    private void drawSingleEvent(Canvas canvas, float xPosition, float yPosition, List<Event> eventsList) {
        ...
        switch(event.getEventType()){
            case(1):
                ...
                break;
            case(2):
                drawSmallIndicatorSquare(canvas, xPosition, yPosition + yIndicatorOffset, event.getColor());
                break;
            case(3):
                drawSmallIndicatorTriangle(canvas, xPosition, yPosition + yIndicatorOffset, event.getColor());
                break;
        }
    ...
    private void drawTwoEvents(Canvas canvas, float xPosition, float yPosition, List<Event> eventsList) {
        ...
        switch(eventsList.get(0).getEventType()) {
            case(1):
                drawSmallIndicatorCircle(canvas, xPosition + (xIndicatorOffset * (-1*smallIndicatorRadiusFactor)), yPosition + yIndicatorOffset, eventsList.get(0).getColor());
                break;
            case(2):
                drawSmallIndicatorSquare(canvas, xPosition + (xIndicatorOffset * (-1*smallIndicatorRadiusFactor)), yPosition + yIndicatorOffset, eventsList.get(0).getColor());
                break;
            case(3):
                drawSmallIndicatorTriangle(canvas, xPosition + (xIndicatorOffset * (-1*smallIndicatorRadiusFactor)), yPosition + yIndicatorOffset, eventsList.get(0).getColor());
                break;
        }

        switch(eventsList.get(1).getEventType()) {
            case(1):
                drawSmallIndicatorCircle(canvas, xPosition + (xIndicatorOffset * smallIndicatorRadiusFactor), yPosition + yIndicatorOffset, eventsList.get(1).getColor());
                break;
            case(2):
                drawSmallIndicatorSquare(canvas, xPosition + (xIndicatorOffset * smallIndicatorRadiusFactor), yPosition + yIndicatorOffset, eventsList.get(1).getColor());
                break;
            case(3):
                drawSmallIndicatorTriangle(canvas, xPosition + (xIndicatorOffset * smallIndicatorRadiusFactor), yPosition + yIndicatorOffset, eventsList.get(1).getColor());
                break;
        }
    ...
    void setTypefaceHeader(Typeface typeface){
        if(typeface!=null) {
            typeface_header=typeface;
        }
    }

    void setTypefaceBody(Typeface typeface) {
        if(typeface!=null) {
            typeface_body=typeface;
        }
    }
        ...
        for (int dayColumn = 0, dayRow = 0; dayColumn <= 6; dayRow++) {
            ...
            if (dayRow == 0) {
                ...
                if (shouldDrawDaysHeader) {
                    dayPaint.setColor(calenderHeaderTextColor);
                    dayPaint.setTypeface(typeface_header);
                    canvas.drawText(dayColumnNames[dayColumn], xPosition, paddingHeight, dayPaint);
                    dayPaint.setTypeface(typeface_body);
                    dayPaint.setColor(calenderTextColor);
                }
            } else {
                ...
                if (day <= monthToDrawCalender.getActualMaximum(Calendar.DAY_OF_MONTH) && day > 0) {
                    ...
                    //novo código para suportar o design de UX
                    canvas.drawText(formatedDay(day), xPosition, yPosition, dayPaint);
                }
            }
        }
    }
    ...
    private void drawSmallIndicatorSquare(Canvas canvas, float x, float y, int color) {
        dayPaint.setColor(color);
        drawSquare(canvas, smallIndicatorRadius, x, y);
    }



    private void drawSmallIndicatorTriangle(Canvas canvas, float x, float y, int color) {
        dayPaint.setColor(color);
        drawTriangle(canvas, smallIndicatorRadius, x, y);
    }
    ...
    private void drawSquare(Canvas canvas, float radius, float x, float y) {
        dayPaint.setStyle(Paint.Style.FILL);
        canvas.drawRect(x-radius,y-radius,x+radius,y+radius,dayPaint);
        dayPaint.setStyle(Paint.Style.STROKE);
        dayPaint.setColor(calenderTextColor);
    }


    private void drawInternalTriangle(Canvas canvas, float radius, float x, float y) {
        float x1 = x;
        float y1 = y-radius;
        float x2 = (float) (x + ((Math.sin(Math.toRadians(120)))*radius));
        float y2 = (float) (y -(Math.cos(Math.toRadians(120))*radius));
        float x3 = (float) (x - ((Math.sin(Math.toRadians(120)))*radius));
        float y3 = y2;
        dayPaint.setStyle(Paint.Style.FILL);
        Path path = new Path();
        path.moveTo(x1,y1);
    ...
    public void drawTriangle(Canvas canvas, float radius, float x, float y) {
        float x1 = x;
        float y1 = y-radius;
        float x2 = x + radius;
        float y2 = y + radius;
